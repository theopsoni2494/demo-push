    ...
    <div className="flex min-h-screen bg-background">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="mx-auto max-w-5xl px-6 py-10">
          ...
          {pastOpen && <PastReplies storeCode={confirmedStore} />}
        </div>
      </main>
  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-r bg-card/60 backdrop-blur md:flex">
      ...
      <nav className="mt-2 flex-1 space-y-1 px-3">
        ...
        >
          <History className="h-4 w-4" /> Past replies
        </button>
      </nav>
    ...
    <div>
      ...
      <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        >
          ...
          <div>
            <p className="text-sm font-semibold text-foreground">Past replies</p>
            <p className="mt-1 text-xs text-muted-foreground">
              View resolved tickets and admin alerts.
            </p>
          </div>
        </button>
      </div>
...
function PastReplies({ storeCode }: { storeCode: string }) {
  ...
  const items = useMemo(() => {
    ...
    const closed = tickets.filter(
      (t) => t.replies.length > 0 && (!code || t.storeCode === code),
    );
  ...
  return (
    <div>
      <h1 className="text-3xl font-semibold tracking-tight">Past replies</h1>
      <p className="mt-2 text-sm text-muted-foreground">
        Resolved tickets and messages sent to your store.
      </p>
